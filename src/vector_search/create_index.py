...
def create_index(features: np.ndarray, 
                 metadata: np.ndarray, 
                 index_name: str):
    """
...
def create_embeddings(image_paths: List[str], 
                      metadata: List[Dict], 
    ...
    for i in tqdm(range(0, len(image_paths), batch_size)):
        ...
        batch_embeddings = extract_image_features(image_batch, model, image_encoder, processor)
...
def main(conf):
    ...
    features = create_embeddings(image_paths, metadata, model, image_encoder, processor, batch_size, image_size)
