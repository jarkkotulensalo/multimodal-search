...
def preprocess_images_error_handling(image_paths: List[str], 
                                     metadata: List[Dict],
                                     image_size: int = 336) -> Tuple[List[PIL.Image.Image], List[str], List[Dict]]:
    """
...
def preprocess_images(image_paths: List[str], 
                      metadata: List[Dict], 
                      image_size: int = 336) -> List[PIL.Image.Image]:
    """
...
def extract_image_features(images: List[PIL.Image.Image], 
    ...
    if image_encoder == 'clip-ViT-B-32-multilingual-v1' or image_encoder == 'clip-ViT-B-32' or image_encoder == 'clip-ViT-L-14' or image_encoder == 'clip-vit-large-patch14-336':
        with torch.no_grad():
...
def extract_metadata_features(metadata: Dict[str, str],
    ...
    metadata_features = model.encode(metadata_text, convert_to_tensor=True, device="cuda" if torch.cuda.is_available() else "cpu")
...
def extract_text_features(text: str, 
    ...
    if text_encoder == 'clip-ViT-B-32-multilingual-v1' or text_encoder == 'clip-ViT-B-32' or text_encoder == 'clip-ViT-L-14' or text_encoder == 'clip-vit-large-patch14-336':
        with torch.no_grad():
        ...
        with torch.no_grad():
            outputs = model.get_text_features(**inputs, output_hidden_states=True, return_dict=True)
